    ...
    panditNS.on("connection", (socket) => {
        ...
        socket.on("ai_query", async ({ text }) => {
            try {
                ...
                socket.emit("ai_response", { text: "🙏 Om Namah Shivay! Please try again in a moment.", sender: "bot" });
            }
        });
...
async function handleCollection(socket, text, state) {
    const steps = {
        name  : { next: 'dob',    msg: "Thank you! Please share your **Date of Birth** (DD-MM-YYYY)." },
        dob   : { next: 'tob',    msg: "Got it. What is your **Time of Birth**? (HH:MM AM/PM)" },
        tob   : { next: 'city',   msg: "What is your **Place of Birth** (City)?" },
        city  : { next: 'gender', msg: "Please share your **Gender** (Male/Female)." },
        gender: { next: 'complete', msg: "🙏 Please wait, calculating your planetary positions..." }
    };
    ...
    if (steps[current].next !== 'complete') {
        ...
        try {
            ...
            const rawData = await generateKundli(
                ...
                city, gender, 5.5, coords.lat, coords.lon
            );
