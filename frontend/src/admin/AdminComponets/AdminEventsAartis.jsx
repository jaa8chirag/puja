...
import {
    Search, Plus, Trash2, X, MapPin, Clock,
    FileText, Image as ImageIcon, Loader2, CheckCircle2, XCircle, LayoutGrid, Info, Navigation
} from "lucide-react";

const ModalField = ({ icon: Icon, placeholder, type = "text", value, onChange, isTextArea }) => (
...
const AdminEventsAartis = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            let endpoint = activeTab === "event" ? "/events" : activeTab === "aarti" ? "/aartis" : "/mandir/all";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.data.success || res.status === 201) {
                ...
                setShowModal(false);
                setFormData({ title: "", location: "", description: "", schedule: "", about: "", map_url: "", image: null });
                fetchData();
            }
        } catch (err) {
    ...
    const deleteItem = async (id) => {
        ...
        try {
            const url = activeTab === 'mandir' ? `${API_BASE_URL}/content/mandir/delete/${id}` : `${API_BASE_URL}/content/delete/${activeTab}/${id}`;
    ...
    return (
        <div className="bg-transparent min-h-screen font-sans p-2">
            {toast && (
                <div className={`fixed top-6 right-6 z-[60] px-5 py-3 rounded-2xl shadow-2xl text-xs font-bold flex items-center gap-3 border animate-in slide-in-from-right-5 ${toast.type === "error" ? "bg-rose-950/40 text-rose-400 border-rose-800/50 backdrop-blur-md" : "bg-emerald-950/40 text-emerald-400 border-emerald-800/50 backdrop-blur-md"
                    }`}>
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-[12px] text-slate-500 font-medium">Manage Events, Aartis & Mandirs</p>
                </div>
                <button
                ...
                <div className="overflow-x-auto">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-3 text-slate-500">
                            ...
                            <span className="text-[10px] uppercase tracking-widest font-bold">Fetching...</span>
                        </div>
                    ) : (
                        <table className="w-full text-xs text-left">
                            ...
                            <tbody className="divide-y divide-slate-800/50">
                                {filteredData.length > 0 ? filteredData.map((item, i) => (
                                    <tr key={item.id || item._id || i} className="hover:bg-[#1a2744] transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="w-12 h-12 rounded-xl border border-slate-700 overflow-hidden bg-[#0f172a]">
                                                {(item.image || item.image_url_1) ? (
                                                    <img src={`${API_BASE_URL}/uploads/${item.image || item.image_url_1}`} className="w-full h-full object-cover" alt="" />
                                                ) : (
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="space-y-1">
                                                <p className="font-bold text-slate-200 text-sm">{item.title || item.name}</p>
                                                <p className="text-slate-500 line-clamp-1 max-w-[200px]">{item.description || item.about}</p>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-1.5 text-slate-400">
                                                <MapPin size={12} className="text-orange-500/60" /> {item.location || "N/A"}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <button onClick={() => deleteItem(item.id || item._id)} className="p-2.5 rounded-xl bg-slate-800 text-slate-400 hover:text-rose-500 transition-all">
                                                <Trash2 size={14} />
                                            </button>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan="5" className="px-6 py-12 text-center text-slate-600 font-bold uppercase tracking-widest text-[10px]">No Records</td></tr>
                                )}
                            </tbody>
                        </table>
                    )}
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
                    <div className="bg-[#131e32] w-full max-w-md rounded-3xl shadow-2xl border border-slate-800 max-h-[90vh] overflow-y-auto">
                        <div className="px-6 py-5 border-b border-slate-800 sticky top-0 bg-[#131e32] z-10 flex justify-between items-center">
                            <h3 className="text-sm font-black text-white uppercase tracking-tight">Add {activeTab}</h3>
                            <button onClick={() => setShowModal(false)} className="text-slate-500 hover:text-white transition"><X size={20} /></button>
                        </div>

                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <ModalField icon={FileText} placeholder={activeTab === 'mandir' ? "Mandir Name *" : `${activeTab} Title *`} value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} />

                            <ModalField icon={MapPin} placeholder="Location" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} />

                            <ModalField icon={Clock}
                                placeholder={activeTab === 'event' ? "Date (e.g. 25 May)" : activeTab === 'aarti' ? "Time (e.g. 7 PM)" : "Timings (e.g. 6AM-9PM)"}
                            ...
                            {activeTab === 'mandir' && (
                                <>
                                    <ModalField icon={Info} placeholder="About (Short)" value={formData.about} onChange={(e) => setFormData({ ...formData, about: e.target.value })} />
                                    <ModalField icon={Navigation} placeholder="Google Maps URL" value={formData.map_url} onChange={(e) => setFormData({ ...formData, map_url: e.target.value })} />
                                </>
                            )}

                            <ModalField icon={FileText} placeholder="Full Description" isTextArea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />

                            <div className="p-3 border border-slate-800 rounded-2xl bg-[#0f172a]">
                                <input type="file" id="fileUpload" className="hidden" onChange={(e) => setFormData({ ...formData, image: e.target.files[0] })} />
                                <label htmlFor="fileUpload" className="flex items-center gap-3 text-[10px] text-slate-400 cursor-pointer">
                                    <ImageIcon size={16} /> {formData.image ? formData.image.name : "Select Image"}
                                </label>
                            </div>

                            <div className="pt-4 flex gap-3">
                                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-3 text-[11px] font-black uppercase rounded-2xl border border-slate-700 text-slate-400">Cancel</button>
                                <button type="submit" disabled={actionLoading} className="flex-1 py-3 text-[11px] font-black uppercase rounded-2xl bg-orange-500 text-white shadow-lg active:scale-95 transition-all">
                                    {actionLoading ? <Loader2 className="animate-spin mx-auto" size={14} /> : "Submit"}
                                </button>
                            </div>
                        </form>
                    </div>
