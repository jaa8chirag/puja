...
import React, { useState, useEffect } from "react";
import {
  X, Trash2, Image, Tag, AlignLeft, IndianRupee, MapPin, Info,
  Clock, Activity, Plus, Sparkles, Edit, Layers, Star // Naye icons
} from "lucide-react";
import {
  API, createBenefit, getBenefitsByService, updateBenefit, deleteBenefit,
} from "../../services/adminApi";
...
const ServiceModal = ({ close, editData, refresh }) => {
  ...
  useEffect(() => {
    if (editData) {
      ...
      setForm({
        ...
        prices: editData.prices?.length > 0
            ? editData.prices
            : [{ pricing_type: "standard", price: "" }],
      });
  ...
  // --- Benefit Handlers ---
  const handleAddBenefitRow = () => setNewBenefits([...newBenefits, { name: "", description: "" }]);
  const handleRemoveBenefitRow = (index) => {
    const updated = newBenefits.filter((_, i) => i !== index);
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex justify-center items-center z-[100] p-4 animate-in fade-in duration-300">
      <div className="bg-[#131e32] w-full max-w-2xl rounded-3xl border border-slate-700/50 shadow-2xl flex flex-col max-h-[95vh] overflow-hidden">
        ...
        <div className="flex justify-between items-center px-8 py-5 border-b border-slate-800 bg-[#0f172a]/50">
          ...
          <button onClick={close} className="p-2 rounded-full hover:bg-slate-800 text-slate-400 transition">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 px-8 py-6 space-y-6 scrollbar-hide">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div 
                  onClick={() => setForm({...form, is_featured: form.is_featured === 1 ? 0 : 1})}
                ...
                >
                  <Star size={14} fill={form.is_featured === 1 ? "currentColor" : "none"} />
          ...
          {isTempleType && (
            <div className="p-6 bg-[#0f172a] rounded-3xl border border-orange-500/20 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                />
                <input
                  type="datetime-local" value={form.dateOfStart}
            ...
            <div className="grid grid-cols-1 gap-3">
              {form.prices.map((p, index) => (
                <div key={index} className="flex items-center gap-2 bg-[#0b1120] p-2 rounded-2xl border border-slate-700">
                  <span className="text-xs text-orange-400 font-bold px-2 capitalize shrink-0 w-20">{p.pricing_type}</span>
                  <input
          ...
          <div className="p-6 bg-gradient-to-br from-purple-950/30 to-blue-950/20 rounded-3xl border border-purple-500/20 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setShowBenefitForm(!showBenefitForm)} className="flex items-center gap-1 px-3 py-1.5 bg-purple-500/20 border border-purple-500/30 rounded-xl text-[10px] font-bold text-purple-300 transition hover:bg-purple-500/30">
            ...
            {showBenefitForm && (
              <div className="p-4 bg-[#0b1120] rounded-2xl border border-purple-500/20 space-y-3">
                {newBenefits.map((benefit, index) => (
                  <div key={index} className="p-3 bg-[#0f172a] rounded-xl border border-slate-700 space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-[9px] font-bold text-slate-500">Benefit #{index + 1}</span>
                      {newBenefits.length > 1 && <X size={12} className="text-red-400 cursor-pointer" onClick={() => handleRemoveBenefitRow(index)} />}
                    </div>
                    <input
                      type="text" value={benefit.name}
                    ...
                    />
                    <textarea
                      rows={2} value={benefit.description}
                ...
                <button type="button" onClick={handleSaveAllBenefits} className="w-full py-2.5 bg-purple-500 rounded-xl text-[11px] font-bold text-white uppercase">
            ...
            <div className="space-y-2">
                {(editData ? benefits : newBenefits.filter(b => b.name)).map((b, i) => (
                    <div key={i} className="p-3 bg-[#0b1120] rounded-xl border border-slate-700 flex justify-between items-center group">
                        <div>
                        ...
                        {editData && (
                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition">
                                <Edit size={14} className="text-purple-400 cursor-pointer" onClick={() => setEditingBenefit(b)} />
                                <Trash2 size={14} className="text-red-400 cursor-pointer" onClick={() => handleDeleteBenefit(b.id)} />
                            </div>
                        )}
                    </div>
                ))}
          ...
          <div className="pt-2">
            <label className="block group cursor-pointer">
              <div className={`border-2 border-dashed rounded-3xl p-6 text-center transition-all ${preview ? "border-orange-500/50 bg-orange-500/5" : "border-slate-800 hover:border-slate-600"}`}>
                {preview ? <img src={preview} className="h-32 mx-auto rounded-2xl shadow-lg" alt="preview" /> : (
                  <div className="text-slate-600 flex flex-col items-center gap-2">
                    <Image size={32} className="opacity-30" />
        ...
        {/* Footer Actions */}
        <div className="px-8 py-6 border-t border-slate-800 flex gap-4 bg-[#0f172a]/50">
          <button onClick={close} className="flex-1 py-4 text-[11px] font-black uppercase text-slate-400 border border-slate-800 rounded-2xl hover:bg-slate-800">Discard</button>
          <button onClick={handleSubmit} className="flex-[2] py-4 text-[11px] font-black uppercase text-white bg-orange-500 rounded-2xl hover:bg-orange-600 shadow-xl shadow-orange-900/40 active:scale-95 transition-all">
            {editData ? "Update Service" : "Deploy Service"}
          </button>
        </div>
      </div>
