...
const StatTile = ({ icon, value, label, iconBg }) => (
  <div
    className="bg-[#161b27] border border-white/[0.06] rounded-xl p-4 md:p-5 flex flex-col gap-2.5 hover:border-white/[0.12] transition-colors"
...
const Adminblogs = () => {
  ...
  const fetchBlogs = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/blogs?${params}`, {
        headers: authHeaders(),
      });
  ...
  const openEdit = async (blog) => {
    setFormError("");
    try {
      const res = await fetch(`${API_BASE_URL}/admin/blogs/${blog.id}`, {
        headers: authHeaders(),
      });
      const data = await res.json();
      const b = data.blog;
    ...
    try {
      ...
      const url =
        modal === "create"
          ...
          : `${API_BASE_URL}/admin/blogs/${selected.id}`;
  ...
  const handleDelete = async () => {
    setSaving(true);
    try {
      const res = await fetch(`${API_BASE_URL}/admin/blogs/${selected.id}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      const data = await res.json();
  ...
  const handleToggleStatus = async (blog) => {
    try {
      const res = await fetch(`${API_BASE_URL}/admin/blogs/${blog.id}/status`, {
        method: "PATCH",
        headers: authHeaders(),
      });
      const data = await res.json();
      ...
      <div className="bg-[#161b27] border border-white/[0.06] rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <button
              onClick={openCreate}
              className="mt-3 text-orange-400 text-sm hover:underline bg-transparent border-none cursor-pointer"
            >
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg overflow-hidden bg-orange-900/20 shrink-0 flex items-center justify-center">
                          {buildImageUrl(blog.image_url) ? (
                            <img
                              src={buildImageUrl(blog.image_url)}
                              alt=""
                        ...
                        <div className="min-w-0">
                          <p className="text-gray-100 font-medium text-[13px] truncate max-w-[200px]">
                            {blog.title}
                          </p>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-white/[0.06]">
            <p className="text-gray-600 text-xs">
              {total} blogs · Page {page}/{totalPages}
            </p>
          ...
          <div className="bg-[#0f1117] border border-white/[0.08] rounded-2xl w-full max-w-2xl shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
              <h2 className="text-gray-100 font-bold text-lg m-0">
                {modal === "create" ? "+ New Blog Post" : "✏️ Edit Blog Post"}
              </h2>
              <button
                onClick={closeModal}
                className="text-gray-500 hover:text-gray-200 text-xl bg-transparent border-none cursor-pointer leading-none"
              >
                ✕
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  placeholder="Write a brief 2-3 line summary..."
