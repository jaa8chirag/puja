...
const STRENGTH_STYLE = {
  VeryStrong: {
    ...
    badge: "bg-emerald-700/60 text-emerald-200",
  },
  Master: {
    bg: "bg-violet-900/50",
    border: "border-violet-500/60",
    text: "text-violet-300",
    badge: "bg-violet-700/60 text-violet-200",
  },
  Strong: {
    bg: "bg-blue-900/40",
    ...
    <div className="flex flex-wrap gap-1 mt-2">
      {letters.map((l, i) =>
        l === " " ? (
          ...
          <div
            key={i}
            className="flex flex-col items-center bg-stone-800/60 rounded-lg px-2 py-1 border border-stone-700/40 min-w-[28px]"
          >
...
function NumberCard({ title, icon, data, name }) {
  ...
  return (
    <div className={`rounded-2xl border p-5 ${st.bg} ${st.border}`}>
      ...
      <div
        className={`inline-block text-xs px-2 py-0.5 rounded-full mb-2 ${st.badge} opacity-80`}
      >
        {data.category}
      </div>
          ...
          <div className="rounded-2xl border border-stone-700/40 bg-stone-900/40 p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {ai.careerAreas.map((c, i) => (
                <span
                  key={i}
                  className="text-xs bg-stone-800 text-stone-300 px-2.5 py-1 rounded-full border border-stone-700/40"
                >
                  {c}
                </span>
              ))}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1">
                  {ai.luckyNumbers.map((n, i) => (
                    <span
                      key={i}
                      className="w-7 h-7 flex items-center justify-center bg-amber-600/30 text-amber-300 rounded-full text-xs font-bold border border-amber-600/30"
                    >
                      {n}
                    </span>
                  ))}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {ai.luckyColors.map((c, i) => (
                    <span
                      key={i}
                      className="text-xs bg-stone-800 text-stone-300 px-2 py-0.5 rounded-full border border-stone-700/30"
                    >
                      {c}
                    </span>
                  ))}
                ...
                <div className="text-center md:text-left flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 justify-center md:justify-start">
                    ...
                    {result.isPowerfulChaldean && (
                      <span className="text-xs bg-violet-900/60 text-violet-300 px-2 py-1 rounded-full">
                        ⭐ Powerful Chaldean
                      </span>
                    )}
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl border border-stone-700/40 bg-stone-900/40 p-5">
                    ...
                    <div className="mt-6 bg-stone-800/50 rounded-xl p-4 text-center">
                      ...
                      <p
                        className={`text-5xl font-black mb-1 ${result.compat.overall >= 80 ? "text-emerald-400" : result.compat.overall >= 65 ? "text-amber-400" : "text-red-400"}`}
                      >
                  ...
                  <div className="rounded-2xl border border-stone-700/40 bg-stone-900/40 p-5 text-sm text-stone-400 space-y-2">
                    {result.compat.overall >= 80 && (
                      <p>
                        ✅{" "}
                        <strong className="text-emerald-400">Excellent</strong>{" "}
                    ...
                    {result.compat.overall >= 65 &&
                      result.compat.overall < 80 && (
                        <p>
                          ⚡ <strong className="text-amber-400">Good</strong> —
                          minor corrections can improve results significantly.
                        </p>
                      )}
                    {result.compat.overall < 65 && (
                      <p>
                        ⚠️ <strong className="text-red-400">Weak</strong> —
