    ...
    {cards.map((card, i) => (
      <div key={i} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "6px" }}>
          ...
          <div style={{
            background: card.severity === 'HIGH' ? '#fee2e2' : card.severity === 'MODERATE' ? '#fef3c7' : '#f0fdf4',
            color: card.severity === 'HIGH' ? '#b91c1c' : card.severity === 'MODERATE' ? '#92400e' : '#166534',
            fontSize: "10px",
            fontWeight: "700",
            padding: "2px 8px",
            borderRadius: "20px",
        ...
        <div style={{ fontSize: "14px", fontWeight: "600", color: "#92400e", marginBottom: "4px" }}>
          🪔 {card.pujaName?.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
        </div>

        {/* Price */}
        {card.price && (
          <div style={{ fontSize: "12px", color: "#15803d", fontWeight: "600", marginBottom: "10px" }}>
            💰 {card.price}
          </div>
        )}
        ...
        >
          {card.severity === 'HIGH' ? '🔴 Book Now — Urgent' : card.severity === 'MODERATE' ? '🟡 Book Puja' : '🟢 Book Puja'}
        </button>
      </div>
    ))}
  </div>
      ...
      {!isOpen && (
        ...
        >
          ...
          <div className="hidden md:block absolute right-24 bg-white/90 backdrop-blur-sm border-l-4 border-amber-600 px-4 py-2 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-all pointer-events-none transform translate-x-4 group-hover:translate-x-0">
            <p className="text-amber-900 font-bold text-sm whitespace-nowrap">🙏 Dear seeker, share your concerns!</p>
          </div>
        </button>
      )}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-[10px] uppercase font-bold text-amber-100">Live</span>
                </div>
              </div>
            </div>
            <button
