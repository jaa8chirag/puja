...
// Dosha → fallback puja ID (agar keyword match na mile)
const DOSHA_FALLBACK_ID = {
  'Mangal Dosha'        : 43,   // mangal dosha
  'Pitra Dosha'         : 48,   // pitra dosha
  'Shani Sade Sati'     : 30,   // Shani Shanti Puja
  'Guru Chandal Yoga'   : 4,    // Navgraha Shanti Puja
  'Shapit Yoga'         : 31,   // Pind Dan Puja
  'Surya Grahan Dosha'  : 4,    // Navgraha Shanti Puja
  'Chandra Grahan Dosha': 3,    // Maha Mrityunjaya Jaap
  'Vish Yoga'           : 6,    // Rudrabhishek
  'Angarak Yoga'        : 7,    // Hanuman Chalisa Path
  'Kaal Sarp Dosh'      : 13,   // Kaal Sarp Dosh Puja
};

// Match a dosha name → puja services array se
      ...
      <div className="relative w-full max-w-[500px] aspect-square">
        <svg viewBox={`0 0 ${S} ${S}`} className="w-full h-full rounded-lg"
          ...
          <text x={M} y={M + 5} textAnchor="middle" fontSize="13" opacity="0.5"
            fill="#5a1a1a" fontFamily="Georgia, serif" fontWeight="bold" letterSpacing="2">Lagna</text>
          ...
          <div className="flex items-center justify-between px-4 pt-4 pb-2">
            <div className="flex items-baseline gap-1">
              <button type="button" onClick={() => setMode("hours")}
                className={`text-3xl font-bold tabular-nums transition-colors ${mode === "hours" ? "text-amber-300" : "text-amber-700 hover:text-amber-500"}`}>{hh}</button>
              <span className="text-amber-600 text-2xl font-bold">:</span>
              <button type="button" onClick={() => setMode("minutes")}
                className={`text-3xl font-bold tabular-nums transition-colors ${mode === "minutes" ? "text-amber-300" : "text-amber-700 hover:text-amber-500"}`}>{mm}</button>
            </div>
              ...
              {["AM", "PM"].map(ap => (
                <button key={ap} type="button" onClick={() => { setAmpm(ap); emit(hrs, mins, ap); }}
                  className={`px-2.5 py-0.5 rounded-lg text-xs font-bold transition-all
                    ${ampm === ap ? "bg-amber-600 text-white" : "text-amber-700 hover:bg-amber-800/40"}`}>{ap}</button>
              ))}
            </div>
          </div>
            ...
            <svg width={SIZE} height={SIZE} onClick={handleClockClick} className="cursor-pointer" style={{ userSelect: "none" }}>
              ...
              {Array.from({ length: 60 }, (_, i) => {
                ...
                return <line key={i} x1={CENTER + r1 * Math.cos(angle)} y1={CENTER + r1 * Math.sin(angle)}
                  ...
                  stroke={isMajor ? "rgba(180,120,30,0.5)" : "rgba(180,120,30,0.2)"} strokeWidth={isMajor ? 1.5 : 1}/>;
              })}
              ...
              {mode === "hours" && hourNumbers.map((n, i) => {
                ...
                return (<g key={n}>
                  ...
                  <text x={pos.x} y={pos.y} textAnchor="middle" dominantBaseline="central" fontSize="13"
                    fontWeight={isSelected ? "bold" : "normal"} fill={isSelected ? "#fde68a" : "rgba(251,191,36,0.7)"}
            ...
            <div className="flex flex-wrap gap-1">
              {[
                ...
                <button key={q.l} type="button"
                  ...
                  className="px-2.5 py-1 text-xs rounded-lg bg-amber-900/40 text-amber-400 hover:bg-amber-700/40 border border-amber-800/30">{q.l}</button>
              ))}
            </div>
          </div>
        </div>
...
// ── Dosha Card — with matched Puja "Book Now" button ─────────
function DoshaCard({ d, matchedPuja }) {
  ...
  return (
    <div className={`border rounded-xl p-4 transition-all ${severityCls}`}>
      ...
      {/* ── SriMandir-style Book Puja Card ── */}
      {matchedPuja && (
        <div className="mt-3 pt-3 border-t border-current/20">
          <a
            ...
            className="group block w-full rounded-2xl overflow-hidden"
          ...
          >
            ...
            {/* ── Book Puja Button ── */}
            <div
              className="w-full py-3 text-center font-bold text-white text-sm flex items-center justify-center gap-2 group-hover:opacity-90 transition-opacity"
...
export default function KundliPortal() {
  ...
  const [allPujas, setAllPujas]   = useState([]);   // ← fetched puja services
  const resultRef = useRef(null);

  // ── Fetch all puja services once on mount ─────────────────
  useEffect(() => {
  ...
  // ── Build doshaName → puja map whenever allPujas or result changes ─
  const doshaPujaMap = useMemo(() => {
    if (!result || allPujas.length === 0) return {};
    const map = {};

    // image_url se full URL banana
    ...
    (result.kundli.doshas || []).forEach(d => {
      ...
      if (matched.length > 0) {
        ...
        map[d.name] = {
          ...
          bookingUrl: `${window.location.origin}/homePuja/${id}`,
        };
      }
    });
    return map;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select name="gender" value={form.gender} onChange={setF}
                ...
                <option className="text-black">Male</option><option className="text-black">Female</option><option className="text-black">Other</option>
              </select>
            </div>
            <div>
          ...
          <div ref={resultRef} className="space-y-5">
            ...
            <div className="bg-amber-950/25 border border-amber-800/30 rounded-2xl p-5 md:p-6 shadow-xl">
              ...
              <div className="mt-6">
                ...
                {tab === 'doshas' && (
                  <div>
                    {/* Summary bar — counts hata diye, sirf heading */}
                    <div className="flex flex-wrap items-center gap-2 mb-5">
                      <h3 className="text-amber-300 font-semibold text-base">⚠️ Dosha Analysis</h3>
                    </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              {result.kundli.doshas
                                ...
                                .map((d, i) => (
                                  // Cancelled doshas mein Book Now nahi dikhega
