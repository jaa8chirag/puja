...
export default function AboutUs() {
  ...
  // ── NEW format: array [{title, content}, ...] ──────────────────────────────
    ...
    return (
      <div className="min-h-screen bg-[#FFF4E1] text-[#2D1A00]">
        <div className="relative max-w-4xl mx-auto px-4 py-8 text-center">
          <p className="text-orange-500/60 text-[10px] tracking-[0.4em] uppercase mb-3 font-bold">
            ॐ
          </p>
          <h1
            className="text-4xl md:text-5xl font-black mb-4 text-[#7c2d00]"
        ...
        <div className="max-w-4xl mx-auto px-4 pb-16 space-y-12">
          {sections.map((sec, i) => (
            <div key={i} className="py-2">
              {sec.title && (
                <h2
                  className="text-xl font-black text-[#7c2d00] mb-3"
                  style={{ fontFamily: "'Georgia', serif" }}
                >
                  {sec.title}
                </h2>
              ...
              {sec.content && (
                <p className="text-orange-900/60 text-sm md:text-base leading-relaxed">
                  {sec.content}
                </p>
              )}
            </div>
  ...
  keys.forEach((key) => {
    ...
    if (key.endsWith("_title")) {
      ...
      if (keys.includes(textKey)) {
        if (s[key] || s[textKey])
          extraSections.push({ title: s[key] || "", text: s[textKey] || "" });
        used.add(key);
        used.add(textKey);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#FFF4E1] text-[#2D1A00]">
      ...
      <div className="max-w-4xl mx-auto px-4 pb-16 space-y-12">
        {/* Stats - Boxes removed, clean grid */}
        {[
          s.stats_pujas,
          s.stats_devotees,
          s.stats_cities,
              ...
              (st, i) =>
                st.val && (
                  <div key={i} className="text-center">
                    ...
                    <p className="text-[10px] font-bold text-orange-700/50 uppercase tracking-widest mt-1">
                      {st.label}
                    </p>
                  </div>
                ),
        ...
        {(s.mission_title || s.mission_text) && (
          <div className="flex gap-6 py-2">
            ...
            <div>
              <h2
                className="text-xl font-black text-[#7c2d00] mb-2"
                style={{ fontFamily: "'Georgia', serif" }}
              >
                {s.mission_title}
              </h2>
              <p className="text-orange-900/60 text-sm leading-relaxed">
                {s.mission_text}
              </p>
            </div>
          </div>
        ...
        {/* Team */}
        {s.team_title && (
          <div className="py-8 text-center border-t border-orange-200/30">
            ...
            <h2
              className="text-2xl font-black text-[#7c2d00] mb-2"
              style={{ fontFamily: "'Georgia', serif" }}
            >
              {s.team_title}
            </h2>
            <p className="text-orange-700/60 text-sm max-w-lg mx-auto">
              {s.team_subtitle}
            </p>
          </div>
        )}
